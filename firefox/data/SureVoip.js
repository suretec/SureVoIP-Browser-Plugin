...
var dialogs = {
    ...
    constructMenu: function () {
        ...
        popupContainer += "<div>";
        popupContainer += "<img id='SureVoIPExtensionImg'  src='" + self.options.logoUrl + "' height='22px' width='112px'/>";
        popupContainer += "</div>";
...
var messageHandler = {
    ...
    init: function () {
        ...
        this.handleMessageFromBackground();
    },
    ...
    sendMessageToBackground: function (action, value) {
        ...
        // });
        self.port.emit(action, value);
        //self.postMessage({action:action,value:value});


    },
    // Receive a message from the background
    handleMessageFromBackground: function () {

        self.port.on(messageHandler.MESSAGES.OpenSettingsDialog, function () {
            dialogs.showSettings();
        });
        self.port.on(messageHandler.MESSAGES.GetSettings_Response, function (msg) {
            console.log("Setting Recieved from background: " + msg);
            if (msg != null || msg != undefined) {
                settings.update(JSON.parse(msg));
            }
        });
        self.port.on(messageHandler.MESSAGES.AjaxSuccess, function (response) {
          
            var msg = response;
            if (msg.status == 202) {
                $(".SureVoIPExtensionLoadingDiv").css("background-image", "url('" + self.options.RequestSentImgUrl + "')");
                $(".SureVoIPExtensionLoadingDiv").fadeOut(3000, function () {
                    $(".SureVoIPExtensionLoadingDiv").css("background-image", "url('" + self.options.SendingRequestImgUrl + "')");
                });
            } else {
                alert("SureVoIP \nServer responded with error code: " + msg.status);
                $(".SureVoIPExtensionLoadingDiv").hide();
            }

        });
        self.port.on(messageHandler.MESSAGES.AjaxError, function (msg) {
            console.log("Message Recieved from background: " + msg);
            alert("SureVoIP \nError sending request to the server.\nError message: " + msg.statusText);
            $(".SureVoIPExtensionLoadingDiv").hide();
        });
    ...
    constructOptionsForFax: function (url, jsonData) {
        var options = {
            ...
            beforeSend: function (data, $form, options) {
                //data.setRequestHeader("Authorization", "Basic "+window.btoa(settings.username+":"+settings.password));
                data.setRequestHeader("Authorization", "Basic " + encodingHelper.encode64(settings.username + ":" + settings.password));
...
//
        ...
        var url = 'https://api.surevoip.co.uk/faxes';
        if (invocation) {
            invocation.open('POST', url, true);
            invocation.withCredentials = true;

            //  invocation.setRequestHeader("Content-Type","multipart/form-data");
            invocation.setRequestHeader("Accept", "application/json ,text/javascript ");
            invocation.onreadystatechange = function (response) {
                console.log(document.readyState);
                if (invocation.readyState == 4) {

                    if (invocation.status != 202) {
                        alert("SureVoIP \nError sending request to the server.\nError message: " + invocation.statusText);
                        $(".SureVoIPExtensionLoadingDiv").hide();
                    } else if (invocation.status == 202) {
                        // alert(response.Fax);
                        // console.log(invocation.statusText +':'+ invocation.status);
                        alert(JSON.parse(invocation.response).Fax);
                        $(".SureVoIPExtensionLoadingDiv").css("background-image", "url('" + self.options.RequestSentImgUrl + "')");
                        $(".SureVoIPExtensionLoadingDiv").fadeOut(3000, function () {
                            $(".SureVoIPExtensionLoadingDiv").css("background-image", "url('" + self.options.SendingRequestImgUrl + "')");
                        });
            ...
            invocation.send(new FormData($('#SureVoIPExtensionFAXDialog')[0]));
        }
...
function init() {
    ...
    $(".SureVoIPExtensionLoadingDiv").css({
        "background-image": "url('" + self.options.SendingRequestImgUrl + " ')",
